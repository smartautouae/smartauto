...
export default function PostsTable({ posts }: { posts: any[] }) {
  ...
  return (
    <div>
      ...
      {/* Table */}
      <div style={{ background: '#fafaf9', border: '1px solid #e8e3d8', borderRadius: '0.75rem', overflow: 'hidden' }}>
        {filtered.length === 0 ? (
              ...
              {filtered.map((post, i) => (
                <tr key={post.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #ede8de' : 'none' }}>
                  ...
                  <td style={{ padding: '0.875rem 1rem' }}>
                    <p style={{ fontSize: '0.82rem', fontWeight: 500, color: '#1a1814', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 280, marginBottom: 2 }}>
                      {post.title}
                    </p>
                      ...
                      {confirmId === post.id ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <button
                            ...
                            style={{ padding: '0.25rem 0.6rem', borderRadius: '0.375rem', fontSize: '0.72rem', fontWeight: 600, background: '#fef2f2', color: '#dc2626', border: '1px solid #fca5a5', cursor: 'pointer' }}
                          >
                            {deleting === post.id ? '…' : 'Confirm'}
                          </button>
                          <button
                            onClick={() => setConfirmId(null)}
                            style={{ padding: '0.25rem 0.6rem', borderRadius: '0.375rem', fontSize: '0.72rem', color: '#7a7264', border: '1px solid #e8e3d8', background: 'transparent', cursor: 'pointer' }}
                          >
                            Cancel
                          </button>
                        </div>
